...
# the ingredient graph, or a registry large enough to blow the heap all surface HERE and
# nowhere else — the unit tests score hand-built profiles and never touch a registry. A
# world-gen pass then runs on the first chunks the server generates, which is the only
# place the loot-box placement code meets a real chunk provider.
#
